...
class Pcap:
    ...
    def _get_layer_dict(layer: Layer):
        ...
        # def local_get_field_tree_old(local_field: LayerField, local_parent: PcapLayerField = None):
        ...
        def update_field_tree_keys(local_field_tree: dict, local_field_tree_keys: list) -> (dict, dict):
            """
            :param local_field_tree: the fields tree (as dict)
            ...
            for local_field_tree_key in local_field_tree_keys:
                local_field_tree = local_field_tree.get(local_field_tree_key)
                if local_field_tree is None:
                    local_field_tree = local_field_tree[local_field_tree_key] = dict()
                if i == local_field_tree_keys_len - 2:
                    local_field_tree_parent = local_field_tree
                i += 1
        ...
        def local_get_field_tree(local_field: LayerField):
            ...
            """
            # dict build with local_field.name
            field_name: str = local_field.name
            field_tree_keys = field_name.split('.')
            parent, node = update_field_tree_keys(field_tree, field_tree_keys)
            ...
            node['field'] = local_pcap_layer_field
            return local_pcap_layer_field

            # print(field_tree)
            # print(field_tree_keys)
            #
            #
            # local_field_insert = field_insert.get(local_field_key)
            # if local_field_insert is not None:
